"""
Este script gera gráficos a partir do ficheiro CSV 'connection_times.csv'.
Ele cria dois tipos de visualizações:
1. Gráfico de Linhas: Mostra a evolução dos tempos de conexão ao longo das execuções para cada base de dados.
2. Gráfico de Barras: Mostra a média e o desvio padrão dos tempos de conexão, ordenado do menor para o maior.
...
Uso:
    python build_graph_connection.py

O script procura pelo ficheiro 'connection_times.csv' no diretório atual.
...
def build_graph_connection():
    csv_file = 'connection_times.csv'
    output_file = 'connection_times_graph.png'
    ...
    try:
        ...
        plt.title('Tempos de Conexão na Base de Dados por Execução', fontsize=16)
        ...
        stats_output_file = 'connection_stats_graph.png'
        ...
        bars = plt.bar(df_sorted.index, df_sorted['Mean'], yerr=df_sorted['StdDev'], capsize=5, color='salmon', alpha=0.9, edgecolor='black')
        
        # Adicionar etiquetas e título
        plt.title('Média e Desvio Padrão dos Tempos de Conexão (Menor para Maior)', fontsize=16)
