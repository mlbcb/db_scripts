"""
Este script gera gráficos a partir do ficheiro CSV 'insert_times.csv'.
Ele cria dois tipos de visualizações:
1. Gráfico de Linhas: Mostra a evolução dos tempos de inserção ao longo das execuções para cada base de dados.
2. Gráfico de Barras: Mostra a média e o desvio padrão dos tempos de inserção, ordenado do menor para o maior.
...
O script procura pelo ficheiro 'insert_times.csv' no diretório atual.
...
def build_graph():
    csv_file = 'insert_times.csv'
    output_file = 'insert_times_graph.png'
    ...
    try:
        ...
        plt.title('Tempos de Inserção na Base de Dados por Execução', fontsize=16)
        ...
        stats_output_file = 'insert_stats_graph.png'
        ...
        bars = plt.bar(df_sorted.index, df_sorted['Mean'], yerr=df_sorted['StdDev'], capsize=5, color='skyblue', alpha=0.9, edgecolor='black')
        
        # Adicionar etiquetas e título
        plt.title('Média e Desvio Padrão dos Tempos de Inserção (Menor para Maior)', fontsize=16)
